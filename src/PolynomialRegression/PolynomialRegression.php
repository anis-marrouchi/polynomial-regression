...
// getForcedCoefficient
    ...
    public function addData($x, $y) {
        ...
        for ($index = 0; $index <= $squares; ++$index) {
            ...
            $this->xPowers[$index] = bcadd($this->xPowers[$index], $accumulator);

            $this->xyPowers[$index] = bcadd($this->xyPowers[$index], bcmul($y, $accumulator));
    ...
    $matrix = array();
        for ($row = 0; $row < $numberOfCoefficient; ++$row) {
            ...
            for ($column = 0; $column < $numberOfCoefficient; ++$column)
                $matrix[$row][$column] = $this->xPowers[$row + $column];
        }
        ...
        for ($column = 0; $column < $rows; ++$column) {
            ...
            // reduced.
            $activeRow = 0;
            while (( ( 0 == $matrix[$activeRow][$column] ) || ( $isDone[$activeRow] ) ) && ( $activeRow < $rows )) {
                ++$activeRow;
            }
            ...
            if ($activeRow < $rows) {
                ...
                for ($subColumn = $column; $subColumn <= $rows; ++$subColumn)
                    $matrix[$activeRow][$subColumn] = bcdiv($matrix[$activeRow][$subColumn], $firstTerm);
    ...
    static public function interpolate($coefficients, $x) {
        ...
        for ($coefficentIndex = 0; $coefficentIndex < $numberOfCoefficient; ++$coefficentIndex) {
            ...
            $y = bcadd
                    ...
                    $y, bcmul
                            (
                            $coefficients[$coefficentIndex], bcpow($x, $coefficentIndex)
                    )
            );
        }
